...
class Scheduler:
	def __init__(self):
		...
		self.chosen_algorithm: str = "FCFS"

	def run(self):
		self.gui.root.mainloop()

	def execute(self):
		if self.chosen_algorithm == "FCFS":
			self.FCFS()
		elif self.chosen_algorithm == "SJF":
			self.SJF()
		elif self.chosen_algorithm == "RR":
			self.RR()
		self.gui.update_chart(self.chart_info)
		self.clear()
	...
	def FCFS(self):
		...
		while self.jobs:
			...
			job_info: JobInfo = JobInfo(job.id, job.name, self.time, execution_time)
	...
	def RR(self):
		...
		while self.jobs or self.queue:
			...
			job_info = JobInfo(job.id, job.name, self.time, execution_time)
			job.state = State.running
			job.burst = max(0, job.burst - execution_time)
